...
def construct_feed_dict(features, support, labels, labels_mask, args):
    """Construct feed dictionary."""
    feed_dict = dict()
    feed_dict.update({args.labels: labels})
    feed_dict.update({args.labels_mask: labels_mask})
    feed_dict.update({args.features: features})

    feed_dict.update({args.support[i]: support[i] for i in range(len(support))})
    feed_dict.update({args.num_features_nonzero: features[1].shape})
    return feed_dict
